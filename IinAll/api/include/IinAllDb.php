...
class IinAllDb
{
   ...
   {
      $light = array();
      $stmt = $this->m_Db->prepare ("SELECT id, text FROM light WHERE text LIKE ? LIMIT 25;");
      $search = "%" . $token . "%";
   ...
   function getRandomLight($count)
   {
      ...
      $stmt = $this->m_Db->prepare ("SELECT id, text FROM light WHERE id IN ($idString);");
      $stmt->execute ();
      $stmt->bind_result ($id, $text);
      while ($stmt->fetch ()) {
   ...
   function getTruth($light)
   {
      ...
      if ($loveId === -1) {
         $truths = array();
         $stmt = $this->m_Db->prepare ("SELECT `truth`.`id`, `truth`.`order`, `truth`.`number`, `truth`.`light_id`, `light`.`text` FROM `truth` INNER JOIN `light` ON `truth`.`light_id`=`light`.`id` WHERE `love_id`=?;");
   ...
   function createLight ($light)
   {
      ...
      if ($id === -1) {
         $stmt = $this->m_Db->prepare ("INSERT INTO light (text) VALUES (?);");
         $stmt->bind_param ("s", $light);
         $stmt->execute ();
         $id = $this->m_Db->getLastInsertId ();
         $stmt->close ();
      }
      return $id;
   }
   ...
   function createLove ($light)
   {
      ...
      if ($id === -1) {
         $stmt = $this->m_Db->prepare ("INSERT INTO love (light) VALUES (?);");
         $stmt->bind_param ("s", $light);
         $stmt->execute ();
         $id = $this->m_Db->getLastInsertId ();
         $stmt->close ();
         ...
         for($i = 0; $i < $idCount; $i++){
            $lightId = $lightIds[$i];
            $stmtP = $this->m_Db->prepare ("INSERT INTO `iinallco_data`.`peace` (`love_id`, `light_id`, `order`) VALUES (?, ?, ?)");
   ...
   {
      $stmt = $this->m_Db->prepare ("SELECT MAX(id) FROM light;");
      $stmt->execute ();
   ...
   {
      $id = -1;
      $stmt = $this->m_Db->prepare ("SELECT id FROM light WHERE text=?;");
      $stmt->bind_param ("s", $light);
      $stmt->execute ();
   ...
   {
      $id = -1;
      $stmt = $this->m_Db->prepare ("SELECT id FROM love WHERE light=?;");
      $stmt->bind_param ("s", $light);
      $stmt->execute ();
   ...
   {
      $stmt = $this->m_Db->prepare ("SELECT id FROM edit_regex WHERE regex=?;");
      $stmt->bind_param ("s", $regex);
      $stmt->execute ();
      $stmt->bind_result ($regexId);
      if (!$stmt->fetch ()) {
         $stmtR = $this->m_Db->prepare ("INSERT INTO edit_regex (regex) VALUES (?);");
   ...
   {
      $regexes= array();
      $stmt = $this->m_Db->prepare ("SELECT id, regex FROM edit_regex;");
      $stmt->execute ();
